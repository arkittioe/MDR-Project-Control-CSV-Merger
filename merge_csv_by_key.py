...
# مسیر فایل‌ها
file1 = r'C:\Users\arkit\Desktop\cp\p05kharg.csv'
...
with open(file1, 'r') as f1, open(output_file, 'w', newline='') as out:
    ...
    writer.writerow(header1 + [''] + header2)  # نوشتن هدر ترکیبی با یک ستون فاصله
    ...
    for row in reader1:
        ...
        if key in data2:
            writer.writerow(row + [''] + data2[key])  # ترکیب ردیف‌های دو فایل با یک ستون فاصله
            matched_keys.add(key)
        else:
            writer.writerow(row + [''] * (len(header2) + 1))  # بدون تغییر اگر کلید در فایل دوم نباشد
